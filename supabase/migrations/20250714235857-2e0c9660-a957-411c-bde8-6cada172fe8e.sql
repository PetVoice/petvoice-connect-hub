-- Fix della funzione RPC per gestire utenti che non esistono ancora in auth.users
...
BEGIN
  ...
  -- Aggiorna referral status E aggiorna referred_user_id se ora esiste
  UPDATE public.referrals 
  SET 
    status = 'converted',
    conversion_date = now(),
    credits_awarded = credit_amount,
    referred_user_id = COALESCE(referred_user_id, referred_user_id), -- Aggiorna se esiste
    updated_at = now()
