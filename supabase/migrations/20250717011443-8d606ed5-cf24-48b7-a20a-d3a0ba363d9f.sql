-- Rimuovi ON DELETE CASCADE dai referral e commissioni per preservare dati storici
-- Passo 1: Rimuovi vincoli esistenti

-- Trova e rimuovi constraint per referrals
...
-- Trova e rimuovi constraint per referral_commissions  
...
-- Per preservare i dati storici quando un utente viene eliminato

-- Referrals table - mantieni i dati storici
ALTER TABLE public.referrals 
...
-- Referral commissions - mantieni i dati storici  
ALTER TABLE public.referral_commissions 
...
-- Passo 5: Aggiorna RLS policies per gestire i dati storici
-- Policy per referrals che include dati storici
...
-- Policy per commissioni che include dati storici
...
USING (
  ...
  (referrer_id IS NULL AND is_historical = TRUE AND EXISTS (
    SELECT 1 FROM public.referrer_stats rs 
    WHERE rs.user_id = auth.uid() AND rs.referrer_email = referral_commissions.referrer_email
  ))
);
