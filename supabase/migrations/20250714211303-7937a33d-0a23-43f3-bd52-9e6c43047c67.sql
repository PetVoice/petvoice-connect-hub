-- Aggiorna il trigger di registrazione referral per inviare email di notifica
  ...
  IF ref_code IS NOT NULL AND ref_code != '' THEN
    ...
    IF referrer_user_id IS NOT NULL THEN
      ...
      IF referrer_email IS NOT NULL THEN
        PERFORM
          net.http_post(
            url := (SELECT url FROM supabase_functions.edge_functions WHERE name = 'send-referral-notification'),
            headers := jsonb_build_object(
              'Content-Type', 'application/json',
              'Authorization', 'Bearer ' || (SELECT service_role_key FROM supabase_functions.secrets WHERE name = 'SUPABASE_SERVICE_ROLE_KEY')
            ),
            body := jsonb_build_object(
              'referrer_email', referrer_email,
              'referrer_name', COALESCE(referrer_name, 'Utente'),
              'referred_email', NEW.email,
              'referral_code', ref_code
            )
          );
