-- RIMUOVI TUTTI I TRIGGER CHE CREANO ABBONAMENTI GRATUITI

-- 1. Drop tutti i trigger sulla tabella auth.users
...
-- 2. Sostituisci con una funzione che crea SOLO profilo senza abbonamento
...
BEGIN
  ...
  -- 2. NON CREARE PIÙ ABBONAMENTI AUTOMATICI!
  
  -- 3. Gestisci referral se presente (solo registrazione, no crediti)
  ...
  IF ref_code IS NOT NULL AND ref_code != '' THEN
    ...
    IF referrer_user_id IS NOT NULL THEN
      INSERT INTO public.referrals (
        ...
        NEW.id,
        ref_code,
        'registered', -- Solo registrato, convertirà quando paga
        'manual_code'
      );
...
-- 3. Crea il nuovo trigger che NON dà abbonamenti gratuiti
...
-- 4. Rimuovi le funzioni vecchie che creano abbonamenti automatici
DROP FUNCTION IF EXISTS public.init_user_subscription();
DROP FUNCTION IF EXISTS public.handle_complete_user_registration();
