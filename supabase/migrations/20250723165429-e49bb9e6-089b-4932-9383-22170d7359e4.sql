-- RICREAZIONE ESERCIZI PER GLI ALTRI 9 PROTOCOLLI (corretto)
...
SELECT 
  ...
  END as title,
  CASE pd.title
    WHEN 'Gestione Ansia da Separazione' THEN 'Lavoro graduale per ridurre ansia durante le separazioni con tecniche di desensibilizzazione progressive.'
  ...
  CASE pd.title
    ...
    WHEN 'Gestione Iperattività e Deficit Attenzione' THEN 
      ARRAY['Esercizi di rilassamento e controllo respiratorio', 'Attività mentali per focalizzare attenzione', 'Giochi strutturati per incanalare energia', 'Training di impulse control con rewards']
    ...
    WHEN 'Rieducazione Alimentare Comportamentale' THEN 
      ARRAY['Stabilire orari fissi per i pasti', 'Utilizzare puzzle feeder per rallentare alimentazione', 'Praticare autocontrollo durante i pasti', 'Creare associazioni positive con il cibo']
