-- RIMUOVI TRIGGER SPECIFICI CHE DIPENDONO DALLE FUNZIONI VECCHIE 
-- Usa CASCADE per forzare la rimozione

-- 1. Rimuovi trigger specifico che dipende da convert_registered_referrals
DROP TRIGGER IF EXISTS convert_registered_to_converted ON public.subscribers CASCADE;

-- 2. Ora rimuovi le funzioni che potrebbero ancora riferirsi a 'subscribed'
...
DROP FUNCTION IF EXISTS public.convert_referral(uuid, text) CASCADE;
DROP FUNCTION IF EXISTS public.process_referral_conversion() CASCADE;

-- 3. Rimuovi tutti gli altri trigger sulla tabella subscribers
DO $$
DECLARE
    trigger_name TEXT;
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS %I ON public.subscribers CASCADE', trigger_name);
    END LOOP;
END $$;

-- 4. Ora ricrea SOLO il trigger necessario per la conversione referral
-- ma usando i nomi colonna corretti (subscription_status invece di subscribed)
