...
import dashboardAnalysisImage from '@/assets/dashboard-analysis.jpg';
import dashboardDiaryImage from '@/assets/dashboard-diary.jpg';
import dashboardCalendarImage from '@/assets/dashboard-calendar.jpg';
import dashboardEmergencyImage from '@/assets/dashboard-emergency.jpg';
import { 
  Heart, 
  PawPrint, 
  Microscope, 
...
const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6 page-enter">
      ...
      {selectedPet && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          {quickActions.map((action, index) => (
            ...
            >
              <CardContent className="p-6 flex flex-col items-center text-center gap-4 relative overflow-hidden">
                {/* Background Image */}
                <div className="absolute inset-0 opacity-10">
                  <img 
                    src={
                      action.title === 'Analisi' ? dashboardAnalysisImage :
                      action.title === 'Diario' ? dashboardDiaryImage :
                      action.title === 'Calendario' ? dashboardCalendarImage :
                      dashboardEmergencyImage
                    } 
      ...
      {/* Health Progress Bar */}
      {selectedPet && (
        <div className="space-y-6 animate-card-up">
          <Card className="border-none shadow-lg bg-gradient-to-r from-cyan-50 to-blue-50 dark:from-cyan-950/20 dark:to-blue-950/20">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-cyan-600 dark:text-cyan-300">
                      Monitoraggio complessivo del benessere
                    </CardDescription>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-bold text-cyan-700 dark:text-cyan-300">
                    {petStats.wellnessScore}%
                  </div>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between text-sm text-muted-foreground">
                  <span>Progresso</span>
                  <span>{petStats.wellnessScore}%</span>
                </div>
                <Progress 
                  value={petStats.wellnessScore} 
                  className="h-3 bg-gradient-to-r from-cyan-100 to-blue-100"
                />
                <div className="grid grid-cols-3 gap-4 pt-2">
                  <div className="text-center">
                    <div className="text-lg font-semibold text-cyan-700 dark:text-cyan-300">{petStats.totalAnalyses}</div>
                    <div className="text-xs text-muted-foreground">Analisi Totali</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold text-blue-700 dark:text-blue-300">{petStats.recentAnalyses}</div>
                    <div className="text-xs text-muted-foreground">Ultime 7 Giorni</div>
                  </div>
                  <div className="text-center">
                    <div className={`text-lg font-semibold ${petStats.moodTrend >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {petStats.moodTrend >= 0 ? '+' : ''}{petStats.moodTrend}%
                    </div>
                    <div className="text-xs text-muted-foreground">Trend Umore</div>
                  </div>
                </div>
              </div>
            </CardContent>
      ...
      {selectedPet && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          ...
          {/* Parametri Vitali Card */}
          <Card className="border-none shadow-lg bg-gradient-to-br from-cyan-50 to-blue-50 dark:from-cyan-950/20 dark:to-blue-950/20 hover:shadow-xl transition-all duration-300 animate-card-up">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-cyan-600 dark:text-cyan-300">
                      {Object.keys(vitalStats).length} parametro{Object.keys(vitalStats).length !== 1 ? 'i' : ''}
                    </CardDescription>
                  </div>
                </div>
                <Button
                  onClick={() => setVitalModal({ open: true, mode: 'add', vitalType: '', currentValue: null })}
                  size="sm"
                  className="h-8 w-8 p-0 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white shadow-lg"
                >
                  <Plus className="h-4 w-4" />
                </Button>
              </div>
            ...
            <CardContent>
              {Object.keys(vitalStats).length > 0 ? (
                  ...
                  <div className="space-y-3">
                    {Object.entries(vitalStats).map(([vital, data]) => {
                      ...
                      const vitalNames: Record<string, string> = {
                        'temperatura': 'Temperatura',
                        'frequenza_cardiaca': 'Battito Card.',
                        'respirazione': 'Respirazione',
                        'colore_gengive': 'Gengive'
                      };

                      // Check if vital is in normal range
                      const rangeCheck = isVitalInNormalRange(vital, data.value, selectedPet?.type);
                      const isNormal = rangeCheck.isNormal;

                      return (
                        <div key={vital} className="flex items-center justify-between p-2 rounded-lg bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm">
                          <div className="flex items-center gap-2">
                            <span className="text-lg">{vitalEmojis[vital] || '📊'}</span>
                            <div>
                              <div className="font-medium text-sm text-gray-900 dark:text-gray-100">
                                {vitalNames[vital] || vital}
                              </div>
                              <div className={`text-xs ${isNormal ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                {typeof data.value === 'string' ? data.value : `${data.value}${data.unit}`}
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-1">
                            ...
                            <div className="flex gap-1">
                              <Button
                                ...
                                size="sm" variant="ghost"
                                className="h-6 w-6 p-0 text-cyan-500 hover:text-cyan-600 hover:bg-cyan-50"
                              >
                                <Edit className="h-3 w-3" />
                              </Button>
                              <Button
                                onClick={() => deleteVitalSign(vital)}
                                size="sm" variant="ghost"
                ...
                <div className="text-center py-6">
                  <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r from-cyan-500/20 to-blue-500/10 flex items-center justify-center">
                    <Activity className="h-8 w-8 text-cyan-500/60" />
                  </div>
                  <p className="text-sm text-muted-foreground mb-4">Nessun parametro registrato</p>
                  <Button
                    onClick={() => setVitalModal({ open: true, mode: 'add', vitalType: '', currentValue: null })}
                    size="sm"
                    className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-semibold px-4 py-2 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300"
                  >
          ...
          {/* Comportamenti Card */}
          <Card className="border-none shadow-lg bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 hover:shadow-xl transition-all duration-300 animate-card-up" style={{ animationDelay: '100ms' }}>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-r from-green-500 to-emerald-500 flex items-center justify-center shadow-lg">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-green-600 dark:text-green-300">
                      {Object.keys(behaviorStats).length} tipo{Object.keys(behaviorStats).length !== 1 ? 'i' : ''}
                    </CardDescription>
                  </div>
                </div>
                <Button
                  onClick={() => setDiaryModal({ open: true, mode: 'add', entry: null })}
                  size="sm"
                  className="h-8 w-8 p-0 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white shadow-lg"
                >
                  <Plus className="h-4 w-4" />
                </Button>
              </div>
            ...
            <CardContent>
              {Object.keys(behaviorStats).length > 0 ? (
                <ScrollArea className="h-32">
                  <div className="space-y-3">
                    {Object.entries(behaviorStats).slice(0, 5).map(([behavior, data]) => (
                      <div key={behavior} className="flex items-center justify-between p-2 rounded-lg bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          <span className="text-xs text-muted-foreground">{data.lastSeen}</span>
                          <Button
                            onClick={() => {
                              const entry = behaviorEntryMap[behavior];
                              if (entry) {
                                setDiaryModal({ open: true, mode: 'edit', entry });
                              }
                            }}
                            size="sm" variant="ghost"
                            className="h-6 w-6 p-0 text-green-500 hover:text-green-600 hover:bg-green-50"
                            disabled={!behaviorEntryMap[behavior]}
                          >
                            <Edit className="h-3 w-3" />
                          </Button>
                        </div>
                ...
                <div className="text-center py-6">
                  <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r from-green-500/20 to-emerald-500/10 flex items-center justify-center">
                    <Brain className="h-8 w-8 text-green-500/60" />
                  </div>
                  <p className="text-sm text-muted-foreground mb-4">Nessun comportamento registrato</p>
                  <Button
                    onClick={() => setDiaryModal({ open: true, mode: 'add', entry: null })}
                    size="sm"
          ...
          {/* Farmaci Card */}
          <Card className="border-none shadow-lg bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-950/20 dark:to-pink-950/20 hover:shadow-xl transition-all duration-300 animate-card-up" style={{ animationDelay: '200ms' }}>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-purple-600 dark:text-purple-300">
                      {medications.length} farmaco{medications.length !== 1 ? 'i' : ''}
                    </CardDescription>
                  </div>
                </div>
                <Button
                  onClick={() => setMedicationModal({ open: true, mode: 'add', medication: null })}
                  size="sm"
                  className="h-8 w-8 p-0 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white shadow-lg"
                >
                  <Plus className="h-4 w-4" />
                </Button>
              </div>
            ...
            <CardContent>
              {medications.length > 0 ? (
                <ScrollArea className="h-32">
                  <div className="space-y-3">
                    {medications.slice(0, 5).map((medication) => (
                      <div key={medication.id} className="flex items-center justify-between p-2 rounded-lg bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm">
                        <div className="flex items-center gap-2">
                          <span className="text-lg">💊</span>
                          <div>
                            <div className="font-medium text-sm text-gray-900 dark:text-gray-100">
                        ...
                        <div className="flex items-center gap-1">
                          <span className="text-xs text-muted-foreground">
                            {format(new Date(medication.last_taken || medication.created_at), 'dd/MM')}
                          </span>
                          <Button
                            onClick={() => setMedicationModal({ open: true, mode: 'edit', medication })}
                            size="sm" variant="ghost"
                ...
                <div className="text-center py-6">
                  <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r from-purple-500/20 to-pink-500/10 flex items-center justify-center">
                    <Pill className="h-8 w-8 text-purple-500/60" />
                  </div>
                  <p className="text-sm text-muted-foreground mb-4">Nessun farmaco registrato</p>
                  <Button
                    onClick={() => setMedicationModal({ open: true, mode: 'add', medication: null })}
                    size="sm"
                    className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-semibold px-4 py-2 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300"
                  >
                    <Plus className="h-4 w-4 mr-2" />
                    Aggiungi Farmaco
                  </Button>
          ...
          {/* Veterinari Card */}
          <Card className="border-none shadow-lg bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-950/20 dark:to-amber-950/20 hover:shadow-xl transition-all duration-300 animate-card-up" style={{ animationDelay: '300ms' }}>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-orange-600 dark:text-orange-300">
                      {veterinaryContacts.length} contatto{veterinaryContacts.length !== 1 ? 'i' : ''}
                    </CardDescription>
                  </div>
                </div>
                <div className="flex gap-1">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="h-8 w-8 p-0 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white shadow-lg"
                  >
                    <Plus className="h-4 w-4" />
                  </Button>
                </div>
            ...
            <CardContent>
              {veterinaryContacts.length > 0 ? (
                <ScrollArea className="h-32">
                  <div className="space-y-3">
                    {veterinaryContacts.slice(0, 5).map((vet) => (
                      <div key={vet.id} className="flex items-center justify-between p-2 rounded-lg bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm">
                        <div className="flex items-center gap-2">
                ...
                <div className="text-center py-6">
                  <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r from-orange-500/20 to-amber-500/10 flex items-center justify-center">
                    <UserCheck className="h-8 w-8 text-orange-500/60" />
                  </div>
                  <p className="text-sm text-muted-foreground mb-4">Nessun veterinario aggiunto</p>
                  <div className="flex gap-2 justify-center">
                    ...
                    <Button
                      onClick={() => setShowVeterinaryModal(true)}
                      size="sm"
                      className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-semibold px-4 py-2 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300"
                    >
                      <Plus className="h-4 w-4 mr-2" />
                      Aggiungi
                    </Button>
          ...
          {/* Visite Recenti Card */}
          <Card className="border-none shadow-lg bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/20 dark:to-purple-950/20 hover:shadow-xl transition-all duration-300 animate-card-up" style={{ animationDelay: '400ms' }}>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-indigo-600 dark:text-indigo-300">
                      {medicalEvents.length} visita{medicalEvents.length !== 1 ? 'e' : ''}
                    </CardDescription>
                  </div>
                </div>
                <Button
                  ...
                  size="sm"
                  className="h-8 w-8 p-0 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white shadow-lg"
                >
                  <Plus className="h-4 w-4" />
                </Button>
              </div>
            ...
            <CardContent>
              {medicalEvents.length > 0 ? (
                <ScrollArea className="h-32">
                  <div className="space-y-3">
                    {medicalEvents.slice(0, 5).map((event) => (
                      <div key={event.id} className="flex items-center justify-between p-2 rounded-lg bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm">
                        <div className="flex items-center gap-2">
                          <span className="text-lg">🏥</span>
                          <div>
                            <div className="font-medium text-sm text-gray-900 dark:text-gray-100">
                        ...
                        <div className="flex items-center gap-1">
                          {event.description && (
                            <Button
                              ...
                              size="sm" variant="ghost"
                              className="h-6 w-6 p-0 text-indigo-500 hover:text-indigo-600 hover:bg-indigo-50"
                            >
                              <Eye className="h-3 w-3" />
                            </Button>
                          )}
                          <Button
                            onClick={() => setEventModal({ open: true, mode: 'edit', event, preselectedCategory: 'medical' })}
                            size="sm" variant="ghost"
                            className="h-6 w-6 p-0 text-indigo-500 hover:text-indigo-600 hover:bg-indigo-50"
                          >
                            <Edit className="h-3 w-3" />
                ...
                <div className="text-center py-6">
                  <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r from-indigo-500/20 to-purple-500/10 flex items-center justify-center">
                    <Stethoscope className="h-8 w-8 text-indigo-500/60" />
                  </div>
                  <p className="text-sm text-muted-foreground mb-4">Nessuna visita registrata</p>
                  <Button
                    onClick={() => setEventModal({ 
                      open: true, 
                      mode: 'add', 
                    ...
                    size="sm"
                    className="bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white font-semibold px-4 py-2 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300"
                  >
                    <Plus className="h-4 w-4 mr-2" />
                    Registra Visita
                  </Button>
