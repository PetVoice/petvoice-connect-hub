        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="faq" className="space-y-6">
           ...
           <TabsContent value="tickets" className="space-y-6">
             {/* Ultimo messaggio */}
             {tickets.length > 0 && (
               <Card className="bg-gradient-to-br from-purple-50/80 to-violet-50/60 border-purple-200/50 shadow-elegant mb-6">
                 <CardContent className="p-4">
                   <div className="flex items-start space-x-3">
                     ...
                     <div className="flex-1">
                       <p className="text-sm text-muted-foreground">Ultimo messaggio</p>
                       {(() => {
                         // Trova il ticket con l'ultimo messaggio
                         let lastMessageTicket = null;
                         ...
                         tickets.forEach(ticket => {
                           ...
                           // Controlla il messaggio iniziale del ticket
                           if (!lastMessageTime || ticketTime > lastMessageTime) {
                             lastMessageTicket = ticket;
                             lastMessageData = { content: ticket.description, type: 'initial', created_at: ticket.created_at };
                             lastMessageTime = ticketTime;
                           }
                           
                           // Controlla le risposte
                           replies.forEach(reply => {
                             const replyTime = new Date(reply.created_at);
                             if (!lastMessageTime || replyTime > lastMessageTime) {
                               lastMessageTicket = ticket;
                               lastMessageData = { content: reply.content, type: 'reply', created_at: reply.created_at, is_staff_reply: reply.is_staff_reply };
                         ...
                         if (!lastMessageData) {
                           return <p className="text-sm">Nessun messaggio ancora</p>;
                         }

                         return (
                           <div>
                             <p className="text-sm font-medium">
                               {lastMessageData.type === 'initial' 
                                 ? `Ticket: ${lastMessageTicket?.subject}` 
                                 : `Risposta ${lastMessageData.is_staff_reply ? 'dal supporto' : 'tua'}`}
                             </p>
                             <p className="text-sm text-muted-foreground mt-1 line-clamp-2">
                               {lastMessageData.content.length > 100 
                                 ? `${lastMessageData.content.substring(0, 100)}...` 
                                 : lastMessageData.content}
                             </p>
                             <p className="text-xs text-muted-foreground mt-2">
                               {formatDistanceToNow(new Date(lastMessageData.created_at), { addSuffix: true, locale: it })}
                             </p>
                           </div>
                         );
                       })()}
